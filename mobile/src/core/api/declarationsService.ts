...
import type { ApiResponse, Declaration, DocTypeCatalog } from '@/types';
import { cache } from '@/utils/cache';

const DOC_TYPES_CACHE_KEY = 'active_doc_types';

export const documentTypesService = {
  async getActive() {
    // Try cache first
    const cached = await cache.get<ApiResponse<DocTypeCatalog[]>>(DOC_TYPES_CACHE_KEY);
    if (cached) {
        // Refresh cache in background
        this.fetchAndCacheActive().catch(() => {});
        return cached;
    }
    return this.fetchAndCacheActive();
  },

  async fetchAndCacheActive() {
    const res = await apiClient.get<ApiResponse<DocTypeCatalog[]>>('document-types/active');
    if (res.data.success && res.data.data) {
        await cache.set(DOC_TYPES_CACHE_KEY, res.data);
    }
    return res.data;
  }
};
...
export const declarationsService = {
  async createLost(data: Record<string, unknown> | FormData) {
    const res = await apiClient.post<ApiResponse<Declaration>>('declarations/lost', data);
    return res.data;
  },

  async createFound(data: Record<string, unknown> | FormData) {
